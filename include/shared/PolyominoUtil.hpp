...
{
    std::array<Polyomino, Config::PolyominoCount> polyominoById;  // All polyominoes indexed by ID
    std::map<Blocks, int, PolyominoComparator> idByPolyomino; // All IDs indexed by Polyomino
    std::array<int, Config::PolyominoCount> clockwiseRotatedIds;
    std::array<int, Config::PolyominoCount> horizontallyReflectedIds;
    std::array<int, Config::PolyominoCount> transformedToCanonical;
};
