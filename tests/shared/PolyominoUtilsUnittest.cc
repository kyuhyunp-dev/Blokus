...
class PolyominoGenerationTest : public PolyominoTestBase 
{}; 

TEST_F(PolyominoGenerationTest, Integrity) {
    for (int i = 0; i < Config::PolyominoCount; ++i) {
        const Blocks& piece = sLibrary.polyominoById[i].blocks;
        ...
        auto it = sLibrary.idByPolyomino.find(piece);

        // Test A: The piece must actually exist in the map
        ASSERT_NE(it, sLibrary.idByPolyomino.end()) 
...
TEST_F(PolyominoGenerationTest, Rotation)
{
    // We track globally visited IDs to ensure every single ID 
    // is part of a valid rotation set.
    std::vector<bool> visited(Config::PolyominoCount, false);
    ...
    for (int i = 0; i < Config::PolyominoCount; ++i) {
        ...
        std::vector<bool> localVisited(Config::PolyominoCount, false);
        ...
        while (!localVisited[currentId]) {
            ...
            currentId = sLibrary.clockwiseRotatedIds[currentId];
        }
...
TEST_F(PolyominoGenerationTest, Reflection) {
    ...
    for (int i = 0; i < Config::PolyominoCount; ++i) 
    {
        ...
        int startId = i;
        int reflectedId = sLibrary.horizontallyReflectedIds[startId];
        ...
        EXPECT_EQ(sLibrary.horizontallyReflectedIds[reflectedId], startId)
            << "Reflection cycle broken: ID " << startId << " -> " << reflectedId 
            << " but " << reflectedId << " -> " << sLibrary.horizontallyReflectedIds[reflectedId];
    }
}

TEST_F(PolyominoGenerationTest, CanonicalMappingIntegrity) {
    for (int i = 0; i < Config::PolyominoCount; ++i) {
        int canonicalId = sLibrary.transformedToCanonical[i];
        ...
        EXPECT_LT(canonicalId, Config::CanonicalCount) 
...
TEST_F(PolyominoGenerationTest, O_Tetromino) 
{
    ...
    int squareId = sLibrary.idByPolyomino.at(square);

    // 1. Test Rotation
    int rotated = sLibrary.clockwiseRotatedIds.at(squareId);
    // After rotation and normalization, it should be identical
    EXPECT_EQ(rotated, squareId) << "Square rotated into a different shape!";

    // 2. Test Reflection
    int reflected = sLibrary.horizontallyReflectedIds.at(squareId);
    EXPECT_EQ(reflected, squareId) << "Square reflected into a different shape!";

    EXPECT_EQ(sLibrary.transformedToCanonical.at(rotated), squareId);
    EXPECT_EQ(sLibrary.transformedToCanonical.at(reflected), squareId);
}

TEST_F(PolyominoGenerationTest, P_Pentomino) 
{
    // 1. Find the ID for the P-Pentomino
    // [(0,0), (1,0), (0,1), (1,1), (0,2)]
    Blocks pShape = {{0, 0}, {1, 0}, {0, 1}, {1, 1}, {0, 2}};
    ASSERT_TRUE(sLibrary.idByPolyomino.count(pShape)) << "P-Pentomino not found in library!";
    
    int startId = sLibrary.idByPolyomino.at(pShape);
    int canonicalId = sLibrary.transformedToCanonical[startId];
    ...
    while (!q.empty()) {
        ...
        EXPECT_EQ(sLibrary.transformedToCanonical[current], canonicalId)
        ...
        std::array<int, 2> neighbors = {
            sLibrary.clockwiseRotatedIds[current],
            sLibrary.horizontallyReflectedIds[current]
        };
        ...
        for (int neighbor : neighbors) {
            ...
            EXPECT_EQ(sLibrary.transformedToCanonical.at(neighbor), startId);
        }
    }
...
TEST_F(PolyominoGenerationTest, MonominoSensorCalculation) {
    // Create a 1x1 piece at (0,0)
    Blocks monomino = { {0, 0} };
    
    // Assume you have a function that generates sensors from blocks
    Polyomino p = sLibrary.polyominoById[sLibrary.idByPolyomino.at(monomino)];
...
TEST_F(PolyominoGenerationTest, RotationConsistency) {
    // Use an L-shape (asymmetric)
    Blocks lBlocks = { {0,0}, {0,1}, {0,2}, {1,2} };
    int id = sLibrary.idByPolyomino.at(lBlocks);

    int rotatedId = sLibrary.clockwiseRotatedIds[id];
    Polyomino rotatedObject = sLibrary.polyominoById.at(rotatedId);

    // Manually calculated rotated edges
    Blocks expectedEdges = { {-1, 0}, {-1, 1}, {0, -1}, {0, 2},
                            {1, -1}, {1, 1}, {2, -1}, {2, 1}, {3, 0}};
    EXPECT_EQ(rotatedObject.sensors.edges, expectedEdges);

    // Manually calculated rotated corners
    Blocks expectedCorners = { {-1, -1}, {-1, 2}, {1, 2}, 
                            {3, -1}, {3, 1} };
    EXPECT_EQ(rotatedObject.sensors.corners, expectedCorners);
}

TEST_F(PolyominoGenerationTest, ReflectionConsistency) {
    // Use an L-shape (asymmetric)
    Blocks lBlocks = { {0,0}, {0,1}, {0,2}, {1,2} };
    int id = sLibrary.idByPolyomino.at(lBlocks);

    int reflectedId = sLibrary.horizontallyReflectedIds[id];
    Polyomino reflectedObject = sLibrary.polyominoById.at(reflectedId);

    // Manually calculated rotated edges
    Blocks expectedEdges = { {-1, 2}, {0, 0}, {0, 1}, {0, 3},
                            {1, -1}, {1, 3}, {2, 0}, {2, 1}, {2, 2}};
    EXPECT_EQ(reflectedObject.sensors.edges, expectedEdges);

    // Manually calculated rotated corners
    Blocks expectedCorners = { {-1, 1}, {-1, 3}, {0, -1}, 
                            {2, -1}, {2, 3} };
    EXPECT_EQ(reflectedObject.sensors.corners, expectedCorners);
}

TEST_F(PolyominoGenerationTest, SensorExclusion) {
    for (int i = 0; i < Config::PolyominoCount; ++i) {
        const auto& p = sLibrary.polyominoById[i];
...
TEST_F(PolyominoGenerationTest, FPentominoAdjacency) {
    // F-Pentomino: A very 'crowded' piece
    Blocks fBlocks = { {1,0}, {2,0}, {0,1}, {1,1}, {1,2} };
    int id = sLibrary.idByPolyomino.at(fBlocks);
    Polyomino fPolyomino = sLibrary.polyominoById[id];
...
TEST_F(PolyominoGenerationTest, SensorBoundingBox) {
    Blocks monomino = { {0,0} };
    Polyomino p = sLibrary.polyominoById[sLibrary.idByPolyomino[monomino]];

    int minX = 0, maxX = 0, minY = 0, maxY = 0;
    
    // Check all sensors (edges and corners)
    auto check_bounds = [&](const Blocks& sensors) {
